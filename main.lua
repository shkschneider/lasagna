-- Main application
-- Game holds world and entities (including player) and coordinates between them.
-- Player reads input and produces intents; Player.apply_physics handles movement.
-- World holds layers and blocks but doesn't know about entities.
-- main.lua's love callbacks call methods on Game, World, and Player appropriately.
...
function love.update(dt)
    ...
    Game.player:apply_physics(dt, Game.world)
...
function love.draw()
    -- World draws layer canvases
    Game.world:draw(Game.camera_x, nil, Game.player.z)

    -- Player draws itself
    if Game.player and Game.player.draw then
    ...
    -- Draw ghost cursor for block placement
