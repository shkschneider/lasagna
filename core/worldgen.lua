...
-- Step 1: Calculate surface height using Perlin noise
function WorldGen.calculate_surface_height(col, z, world_height)
...
-- Step 2: Fill terrain with air above surface and stone below
function WorldGen.fill_base_terrain(layers, z, col, base_height, world_height)
...
-- Step 3: Place bedrock layer at the bottom
function WorldGen.place_bedrock(layers, z, col, world_height)
    layers[z][col][world_height - 2] = BLOCKS.BEDROCK
    layers[z][col][world_height - 1] = BLOCKS.BEDROCK
end

-- Step 4: Generate ore veins using 3D Perlin noise
function WorldGen.generate_ore_veins(layers, z, col, base_height, world_height)
...
-- Step 5: Add dirt layer on top of stone
function WorldGen.add_dirt_layer(layers, z, col, base_height, world_height)
...
-- Step 6: Place grass on topmost dirt
function WorldGen.place_grass(layers, z, col, base_height, world_height)
...
function WorldGen.generate_column(layers, z, col, world_height)
    -- Step 1: Calculate surface height
    local base_height = WorldGen.calculate_surface_height(col, z, world_height)
    
    -- Step 2: Fill base terrain (air and stone)
    WorldGen.fill_base_terrain(layers, z, col, base_height, world_height)
    
    -- Step 3: Place bedrock at bottom
    WorldGen.place_bedrock(layers, z, col, world_height)
    
    -- Step 4: Generate ore veins
    WorldGen.generate_ore_veins(layers, z, col, base_height, world_height)
    
    -- Step 5: Add dirt layer
    WorldGen.add_dirt_layer(layers, z, col, base_height, world_height)
    
    -- Step 6: Place grass
    WorldGen.place_grass(layers, z, col, base_height, world_height)
end
