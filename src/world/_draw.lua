...
-- Cutout mask parameters
local CUTOUT_RADIUS = 64  -- pixels, player torso zone
local CUTOUT_FEATHER = 12  -- pixels, soft edge blur
local FADE_ALPHA = 0.35  -- fade opacity for front layer

-- Draw with cutout mask and fade effect
local function draw_with_cutout(self, layer, pz)
    -- Get current screen dimensions dynamically
    local screen_width, screen_height = love.graphics.getDimensions()
    ...
    -- Draw blocks using actual block colors with cutout and fade
    for col = start_col, end_col do
        for row = start_row, end_row do
            local value = self:get_block_value(layer, col, row)
            ...
            elseif value == BlockRef.AIR then
                ...
                if block_id then
                    ...
                    if block and block.color then
                        ...
                        if dist < CUTOUT_RADIUS then
                            cutout_alpha = 0.0  -- Hidden in core
                        elseif dist < (CUTOUT_RADIUS + CUTOUT_FEATHER) then
                            -- Smooth feather from 0 to 1
                            local feather_t = (dist - CUTOUT_RADIUS) / CUTOUT_FEATHER
                            cutout_alpha = feather_t
                        end

                        -- Combine cutout and fade alpha
                        local final_alpha = cutout_alpha * FADE_ALPHA * (block.color[4] or 1)
...
function World.draw3(self, pz)
    -- Draw only the immediate front layer (1 layer above player)
    local front_layer = math.min(pz + 1, LAYER_MAX)
    if front_layer > pz then
        draw_with_cutout(self, front_layer, pz)
    end
end
