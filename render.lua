...
function render.create_canvases(self)
...
function render.draw_world(self, w, player_layer, camera_x, camera_y)
    local start_col = math.floor(camera_x / world.BLOCK_SIZE) - 1
    local end_col = math.ceil((camera_x + self.screen_width) / world.BLOCK_SIZE) + 1
    local start_row = math.floor(camera_y / world.BLOCK_SIZE) - 1
    local end_row = math.ceil((camera_y + self.screen_height) / world.BLOCK_SIZE) + 1
...
function render.composite_layers(self, player_layer)
    -- Clear screen
...
function render.draw_ui(self, p, w, camera_x, camera_y)
    local inv = p.inventory
    if not inv then return end

    -- Draw hotbar
    local hotbar_y = self.screen_height - 60
    local slot_size = 50
    local hotbar_width = inventory.HOTBAR_SIZE * slot_size
    local hotbar_x = (self.screen_width - hotbar_width) / 2
