...
local function deserialize_storage(storage, saved_data)
    ...
    for i = 1, storage.size do
        ...
        if slot_data then
            ...
            storage.slots[i] = StackComponent.new(id, slot_data.count, id_type)
        else
            storage.slots[i] = nil
        end
    end
...
function SaveSystem.create_save_data(self)
    local save_data = {
...
function SaveSystem.apply_save_data(self, save_data)
    if not save_data then
        Log.error("SaveSystem", "No save data to apply")
        return false
    end
    ...
    if save_data.version then
        ...
        if saved.major ~= current.major or saved.minor ~= current.minor then
            Log.warn("SaveSystem", string.format(
    ...
    Log.info("SaveSystem", "Save data applied successfully")
    return true
end
...
function SaveSystem.load(self)
    ...
    Log.info("SaveSystem", "Save loaded from " .. path)
    return save_data
end

-- Check if a save file exists
function SaveSystem.exists(self)
    local path = self:get_save_path()
    return love.filesystem.getInfo(path) ~= nil
end

-- Delete save file
function SaveSystem.delete(self)
    local path = self:get_save_path()
