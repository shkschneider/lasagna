...
function CameraSystem:load(x, y)
    self.components.camera = Camera.new(x, y, x, y, 5)
    self.screen_width = 1280
    self.screen_height = 720
end

function CameraSystem:update(dt, target_x, target_y)
...
function CameraSystem:get_offset()
    local cam = self.components.camera
    return cam.x - self.screen_width / 2, cam.y - self.screen_height / 2
end

function CameraSystem:resize(width, height)
